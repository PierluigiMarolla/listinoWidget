...
const ListinoTab = ({ close, open, openSet, setValue }) => {
  ...
  const DetailComponent = (props) => {
    ...
    const handleSicurezzaChange = (event) => {
      setSicurezza(event.target.value);
    };

    const handleManodoperaChange = (event) => {
      setManodopera(event.target.value);
    };

    const handleAttrezzatureChange = (event) => {
      setAttrezzature(event.target.value);
    };

    const handleMaterialiChange = (event) => {
      setMateriali(event.target.value);
    };

    const p1Changer = (value) => {
      setP1(value)
      setUsaAnalisi(true)
    }
    ...
    return (
      <>
        ...
        <section className={tabAnalisiOpen ? "tabOpen" : "tabClosed"}>
          ...
          <AutoCompleteSelect p1Changer={p1Changer} />
        </section>
      </>
    );
  };
